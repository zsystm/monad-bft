...
async fn test_tcp_limits_are_applied(
    ...
    let mut tx1 = DataplaneBuilder::new(&tx1_addr, UP_BANDWIDTH_MBPS).build();
    let mut tx2 = DataplaneBuilder::new(&tx2_addr, UP_BANDWIDTH_MBPS).build();
    ...
    let (_, received_payload) = rx.tcp_read().await;
    assert_eq!(received_payload, payload1);
...
async fn test_tcp_rps_limits() {
    ...
    for i in 0..2 {
        ...
        assert_eq!(received_payload, expected);
    }
...
async fn test_tcp_limits_ignored_for_trusted(
    ...
    let (_, received_payload) = rx.tcp_read().await;
    assert_eq!(received_payload, payload2);
}
