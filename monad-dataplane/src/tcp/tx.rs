...
async fn task_peer(tx_state: TxState, conn_id: u64, addr: SocketAddr) {
    ...
    if let Err(err) = connect_and_send_all_messages(&tx_state, conn_id, &addr).await {
        ...
        tx_state.inner.borrow_mut().messages.remove(&addr);
    }
...
async fn connect_and_send_all_messages(
    tx_state: &TxState,
    conn_id: u64,
    addr: &SocketAddr,
) -> Result<(), Error> {
