...
//! an iterator and provide a [`TypedEventReader::next`] method which produces
...
//! [`TypedEventReader::next`] will eventually produce the event written. Unlike iterators however,
//! event rings are backed by a fixed-size descriptor array and a fixed-size payload array, both of
//! which can be overwritten if the current process is unable to consume events at the same rate
//! they are being produced and falls behind. In this case, the user is notified through the
//! [`EventNextResult::Gap`] variant. Once an event descriptor or payload is overwritten, it is
//! **unrecoverable**. Programs that depend on consuming all events of some kind produced by an
//! event ring **must** enter a recovery phase to recover the (possibly) missing data.
//!
//! <div class="warning">
//!
//! Production applications should **never** rely on the [`EventReader`] keeping up and **must**
//! implement a recovery phase which should be triggered when the event ring gaps. This mecahnism
...
//! </div>
//!
//! Unlike [`TypedEventRing`]s, [`TypedEventReader`]s are single threaded as they use the lifetime
