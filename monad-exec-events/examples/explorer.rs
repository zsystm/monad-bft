...
use monad_exec_events::{
    monad_exec_block_header, monad_exec_block_result, BlockBuilderResult, BlockCommitState,
    ConsensusStateTracker, ConsensusStateTrackerUpdate, ExecEventRingType, ExecutedBlock,
    ExecutedTxn,
};
use ratatui::{
...
struct App {
    ...
    overview_frozen_buffer: Option<Vec<BlockBuilderResult<ConsensusStateTrackerUpdate>>>,
...
impl App {
    ...
    fn run(mut self, mut terminal: DefaultTerminal) -> Result<(), Box<dyn std::error::Error>> {
        let event_ring =
            EventRing::<ExecEventRingType>::new_from_path(&self.event_ring_path).unwrap();
        ...
        loop {
            loop {
                ...
                let Some(result) =
                    consensus_state_tracker.process_event_descriptor(&event_descriptor)
                else {
                    continue;
                };
    ...
    fn process_state_result(&mut self, result: BlockBuilderResult<ConsensusStateTrackerUpdate>) {
        match result {
            ...
            BlockBuilderResult::Ok(ConsensusStateTrackerUpdate {
                ...
                match self.rounds.entry(block.header.proposal.round) {
                    BTreeMapEntry::Vacant(v) => {
                        let was_selecting_last = self
                            .overview_table_state
                            .selected()
                            .map_or(false, |selected_row| {
                ...
                if let Some(abandoned) = abandoned {
                    for abandoned in abandoned {
                        self.rounds
                            .get_mut(&abandoned.header.proposal.round)
                            .unwrap()
                            .0 = None;
                    }
                }
    ...
    fn render_overview_table(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let rows = self.rounds.iter().map(|(round, (state, block))| {
            [
                ...
                Text::from(hex::encode(block.header.proposal.id.bytes)),
                Text::from(
                    Span::raw(block.header.proposal.block_number.to_string()).style(
                        if state.is_none() {
                ...
                Text::from(
                    DateTime::from_timestamp(
                        block.header.exec_input.timestamp.try_into().unwrap(),
                        0,
                    )
                    .unwrap()
                    .to_rfc3339(),
                ),
                Text::from(block.header.exec_input.txn_count.to_string()),
            ]
            .into_iter()
            .map(|mut text| {
                if state.is_none() {
    ...
    fn render_overview_scrollbar(&mut self, frame: &mut Frame, area: Rect) {
        let mut scrollbar_area = area.clone();
        scrollbar_area.y += 3;
    ...
    fn render_block_header(
        ...
        let rows: Vec<(&'static str, String, Box<[u8]>)> = vec![
            (
                ...
                proposal.block_number.to_string(),
                Box::new(proposal.block_number.to_be_bytes()),
            ),
            (
                "proposal.epoch",
                proposal.epoch.to_string(),
                Box::new(proposal.epoch.to_be_bytes()),
            ),
            (
                "proposal.id",
                hex::encode(proposal.id.bytes),
                Box::new(proposal.id.bytes),
            ),
            (
                "proposal.parent_id",
                hex::encode(proposal.parent_id.bytes),
                Box::new(proposal.parent_id.bytes),
            ),
            (
                "proposal.parent_round",
                proposal.parent_round.to_string(),
                Box::new(proposal.parent_round.to_be_bytes()),
            ),
            (
                "proposal.round",
                proposal.round.to_string(),
                Box::new(proposal.round.to_be_bytes()),
            ),
            (
                "parent_eth_hash",
    ...
    fn render_block_execution_result(
        ...
        area: Rect,
        table_state: &mut TableState,
        block_result: &monad_exec_block_result,
    ) {
        let monad_exec_block_result {
            eth_block_hash,
            exec_output,
        } = block_result;
