...
use monad_event_ring::{
    EventDescriptorPayload, EventNextResult, EventRing, TypedEventDescriptor, TypedEventReader,
    TypedEventRing,
};
...
fn main() {
    ...
    loop {
        ...
        // Note: We should NOT call println inside `try_map_raw` as it is possible the bytes are
        // overwritten and the event is thus "Expired" before the println is complete which would
        // lead to invalid bytes being dumped.
        let (info, hexdump) = match event.try_filter_map_raw(|info, bytes| {
            Some((
                info,
                bytes
                    .iter()
