...
impl<'ring> ExecEventStream<'ring> {
    ...
    fn act_on_txn_receipt(&'_ mut self, event: monad_event_descriptor) -> PollResult {
        let payload = self.reader.payload_peek(&event);
        let txn_receipt = unsafe { &*(payload.as_ptr() as *const exec_event_ctypes::txn_receipt) };
        let txn_no = get_flow_info(&event).txn_id - 1;
        self.try_create_event(
            event,
            ExecEvent::TransactionReceipt {
                txn_index: txn_no as u64,
                status: alloy_consensus::Eip658Value::Eip658(txn_receipt.receipt.status),
                log_count: txn_receipt.receipt.log_count as usize,
                call_frame_count: txn_receipt.call_frame_count as usize,
                txn_gas_used: txn_receipt.receipt.gas_used as u128,
            },
        )
    }
    ...
    fn act_on_storage_access(&'_ mut self, event: monad_event_descriptor) -> PollResult {
        ...
        self.try_create_event(
            ...
            ExecEvent::StorageAccess {
                ...
                account_index: event.user[1] as u32,
    ...
    fn act_on_evm_error(&'_ mut self, event: monad_event_descriptor) -> PollResult {
        ...
        self.try_create_event(
            ...
            ExecEvent::EvmError {
                ...
                txn_id: get_flow_info(&event).txn_id as u64,
            },
        )
    }
...
fn make_account_access_context(
    ...
    let txn_id = get_flow_info(event).txn_id as u64;
    match context_code {
