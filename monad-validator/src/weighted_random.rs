...
#[derive(Clone, Debug)]
pub struct WeightedRandomLeaderSelection {
    voters: Vec<Voter>,
}

impl LeaderElection for WeightedRandomLeaderSelection {
    fn new() -> Self {
        Self { voters: Vec::new(), }
    }

    fn get_leader(&self, round: Round, _validator_list: &[NodeId]) -> NodeId {
        ...
        self.panic_if_empty();//
        let seed = round.0 as u64;
        let mut rng = ChaCha20Rng::seed_from_u64(seed);

        let stakes: Vec<u64> = self.voters.iter().map(|v| v.voting_power.0 as u64).collect(); 
        ...
        if let Ok(dist) = WeightedIndex::new(&stakes) {
            ...
            panic!("No valid leader can be selected due to distribution creation failure."); // FIXME: Not sure if we want panic here or return a result type( Result<NodeId, &'static str> ) and let the caller decide on what to do in this case. 
...
impl WeightedRandomLeaderSelection {
    fn panic_if_empty(&self) {
        if self.voters.is_empty() {
            panic!("Voter list is empty, cannot select a leader.");
        }
    }
    pub fn start_new_epoch(&mut self, voting_powers: Vec<(NodeId, Stake)>) {
        self.voters.clear();
        for (addr, vp) in voting_powers.into_iter() {
            let voter = Voter { address: addr, voting_power: vp };
            if voter.verified() {
...
mod tests {
    ...
    fn generate_unique_node_id(input: u8) -> NodeId {
    ...
    fn test_new_weighted_random_leader_selection() {
        let selection = WeightedRandomLeaderSelection::new();
    ...
    fn test_add_voters_and_select_leader() {
        ...
        let mut selection = WeightedRandomLeaderSelection::new();
        ...
        let leader = selection.get_leader(Round(4), &[]);
    ...
    fn test_leader_selection_with_zero_stakes() {
        ...
        let leader = selection.get_leader(Round(1), &[]);
        println!("Number of voters after start_new_epoch: {}", selection.voters.len());
    ...
    fn test_leader_election_distribution() {
        ...
        for _ in 0..total_iterations {
            ...
            let leader = selection.get_leader(round, &voters.iter().map(|(id, _)| *id).collect::<Vec<_>>());
        ...
        for (node_id, stake) in &voters {
            ...
            assert!((stake_ratio - election_ratio).abs() < 0.1, "Election frequency does not match stake distribution closely enough.");
        }
    }
