...
impl<'k, ST: CertificateSignatureRecoverable> Chunker<'k> for Raptor<'k, ST> {
    ...
    fn new_from_message(
        ...
        let raptor_symbol_size = MAX_DATAGRAM_SIZE - 150 - 70;
            ...
            Role::Encoder {
                ...
                if pending_chunks.is_empty() {
                    let packets = {
                        ...
                        const PACKET_BATCH: u8 = 8; // power of 2
                        while packets.len() < PACKET_BATCH.into() {
...
impl<ST: CertificateSignatureRecoverable> RaptorChunk<ST> {
    fn compute_merkle_root(&self, chunk: &Bytes) -> Option<MerkleHash> {
        let mut merkle_hash = hash_to_merkle(&Self::compute_payload_hash(&self.payload_id, chunk));
        let mut current_idx = Some(self.merkle_leaf_idx as usize);

        for sibling in self.merkle_proof.iter().rev() {
    ...
    pub fn create(
        ...
        assert!(chunks.len() <= 64); // make sure all tree idx fit in u16

        let num_leaves = chunks.len().next_power_of_two();
        let merkle_tree_len = 2 * num_leaves - 1;
        let mut merkle_tree = vec![MerkleHash::default(); merkle_tree_len];
        for (leaf, chunk) in merkle_tree[merkle_tree_len - num_leaves..]
            .iter_mut()
            .zip(chunks.iter())
        {
            *leaf = hash_to_merkle(&Self::compute_payload_hash(&payload_id, chunk));
        }
        for idx in (0..merkle_tree.len() - num_leaves).rev() {
            let mut h = HasherType::new();
            h.update(merkle_tree[2 * idx + 1]);
            h.update(merkle_tree[2 * idx + 2]);
            merkle_tree[idx] = hash_to_merkle(&h.hash());
        }

        // TODO add prefix so this is safe and signature can't be re-used?
        let root_signature = ST::sign(&merkle_tree[0], key);

        chunks
            .into_iter()
            .enumerate()
            .map(|(idx, chunk)| {
                let merkle_idx = merkle_tree_len - num_leaves + idx;
                let raptor_chunk = Self {
                    payload_id,
                    merkle_leaf_idx: merkle_idx.try_into().expect("length was checked above"),
                    merkle_proof: {
                        let mut proof = Vec::new();
                        let mut current_idx = merkle_idx;
                        while let Some(sibling_idx) = sibling_idx(current_idx) {
                            current_idx =
                                parent_idx(current_idx).expect("if has sibling, must have parent");
                            proof.push(merkle_tree[sibling_idx]);
                        }
                        proof.reverse();
                        proof
                    },
