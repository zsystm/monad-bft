    ...
    SVT: StateRootValidator,
{
        ...
        TT: TxPool,
    {
        ...
        if !self.randao_validation(&block, author, epoch, node.val_epoch_map, node.metrics) {
            return cmds;
        }
        ...
        let proposer_builder =
            |txns: FullTransactionList,
             ...
             last_round_tc: Option<TimeoutCertificate<SCT>>| {
                ...
                let validator_monitor: ValidatorMonitor<SCT::NodeIdPubKey,SCT> = ValidatorMonitor::new(); // Initialize validator_monitor before using it
...
mod test {
    ...
    fn test_unavailable_state_root_during_proposal() {
        ...
        // to be unavailable
        let (mut env, mut ctx) =
            setup::<SignatureType, SignatureCollectionType, _, MissingNextStateRoot, _, _>(
                num_state,
                ValidatorSetFactory::default(),
                SimpleRoundRobin::default(),
                MockTxPool::default(),
