...
async fn ensure_disk_space(
    ...
    let need = if let Ok(coll_storage_size) = get_int_field(&coll_stats, "storageSize") {
        // Use collection-specific storage size if available
        (coll_storage_size as f64 * factor) as i64
    } else {
        ...
        ((estimate_base as f64 * 0.1 * factor) as i64).max(1_000_000_000) // At least 1GB
    };
...
async fn copy_batches(
    ...
    // Collect the slice of _ids (cheap – _id is small)
    let mut ids: Vec<mongodb::bson::Bson> = Vec::with_capacity(batch as usize);
