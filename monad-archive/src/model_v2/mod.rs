...
impl LatestDoc {
    ...
    #[allow(dead_code)]
    pub fn receipt_eviction(block_number: i64) -> Self {
        Self {
            _id: "receipt_eviction".to_string(),
            block_number,
        }
    }

    #[allow(dead_code)]
    pub fn tx_body_eviction(block_number: i64) -> Self {
        Self {
            _id: "tx_body_eviction".to_string(),
            block_number,
        }
    }
    ...
    #[allow(dead_code)]
    pub fn receipt_eviction_query() -> Document {
        doc! { "_id": "receipt_eviction" }
    }

    #[allow(dead_code)]
    pub fn tx_body_eviction_query() -> Document {
        doc! { "_id": "tx_body_eviction" }
    }
}
...
impl ModelV2 {
    pub async fn from_client(
        ...
        if !collection_names.contains(&"txs".to_string()) {
            ...
            .await?;
            info!("txs collection and indexes created");
            // TODO: eth_getLogs indexes in future pr
        }
...
impl BlockArchiver for ModelV2 {
    fn object_store(&self) -> KVStoreErased {
        // TODO: This method is used for migration purposes.
        // Since we now use Box<dyn ObjectStore>, we can't easily convert to KVStoreErased.
        // For now, we'll panic if this is called, as V2 doesn't support the old migration path.
        panic!("ModelV2 does not support object_store() - use direct S3 access instead")
    }
...
/// Spawns a future that outputs an eyre::Result into a tokio task and flattens the task and future results into 1 eyre::Result
/// Note: cannot use generic error due to eyre reliance on a private trait that cannot be in bound
async fn spawn_eyre<T: Future<Output = Result<O>> + Send + 'static, O: Send + 'static>(
    req: T,
    msg: &'static str,
) -> Result<O> {
...
mod tests {
    ...
    pub async fn setup() -> Result<(TestMongoContainer, ModelV2)> {
        ...
        let object_store = ObjectStore::Memory(memory_storage);

        let model_v2 =
            ModelV2::from_client(client, object_store, 10, "test_db".to_string()).await?;
