...
mod tests {
    ...
    async fn test_eviction() -> Result<()> {
        ...
        v1_model.archive_block(block.clone()).await?;
        v1_model.archive_receipts(receipts, block_num).await?;
        v1_model.archive_traces(traces.clone(), block_num).await?;
    ...
    async fn test_eviction_with_size_threshold() -> Result<()> {
        ...
        loop {
            ...
            model
                .archive_block_data(block.clone(), receipts.clone(), traces.clone())
                .await?;
            v1_model.archive_block(block).await?;
            v1_model.archive_receipts(receipts, block_num).await?;
            v1_model.archive_traces(traces, block_num).await?;
