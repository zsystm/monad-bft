...
use crate::{BlockDataReader, LatestKind, Metrics};

use super::block_data_archive::BlockDataArchive;
...
mod tests {
    use alloy_consensus::{
        BlockBody, Header, Receipt, ReceiptEnvelope, ReceiptWithBloom, SignableTransaction, Signed,
        TxEip1559, TxEnvelope,
    };
    use alloy_primitives::{keccak256, Address, Bloom, Log};
    ...
    use crate::{
        ...
        BlobStore, BlobStoreErased, BlockDataReader,
    };
    ...
    async fn archive_blocks_with_gap() {
        ...
        mock_source(
            &reader,
            (0..=end_of_first_chunk).map(row).chain((12..=latest_source).map(row)),
        )
        .await;
        ...
        assert_eq!(archiver.get_latest(LatestKind::Uploaded).await.unwrap(), end_of_first_chunk);
    }
}
