...
impl Keystore {
    ...
    pub fn encrypt(&self, private_key: &Vec<u8>, password: &String) -> Result<(Vec<u8>, Vec<u8>), KeystoreError> {
        ...
        let checksum = self.checksum.generate_checksum(&encryption_key, &ciphertext);
...
mod test {
    ...
    fn test_keystore_encrypt() {
        ...
        let keystore = Keystore::new(&serialized_json).expect("invalid json format");

        let result = keystore.encrypt(&hex::decode(sk).unwrap(), &password).expect("encryption failed");
    ...
    fn test_keystore_decrypt() {
        ...
        let sk = keystore.decrypt(&password).expect("decryption failed");
