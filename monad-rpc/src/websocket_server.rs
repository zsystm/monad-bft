...
use crate::{
    eth_json_types::{
        BlockCommitState, EthSubscribeResult, EventStreamItem, FixedData, SpeculativeLog, SpeculativeNewHead,
...
impl WebSocketServer {
                ...
                Ok(poll_result) = self.rx.recv_async() => {
                    match poll_result {
                        ...
                        PollResult::Gap { last_read_seqno, last_write_seqno } => {
                            ...
                            self.process_event_stream_item(EventStreamItem::StreamGap {
                                last_read_seqno,
                        ...
                        PollResult::Ready{ seqno, event } => {
                            if let ExecEvent::Referendum { proposal_meta, outcome } = &event {
                                if let ConsensusStateResult::Finalization{ finalized_proposal: _, abandoned_proposals } = self.consensus_state_tracker.update_proposal(proposal_meta.block_number, &proposal_meta.id, *outcome) {
                                    for abandoned in abandoned_proposals {
                                        ...
                                        self.process_event_stream_item(EventStreamItem::AbandonedProposal {
                                            proposal_meta: abandoned.proposal_meta.clone()
                                        });
                                    }
                                }
                            ...
                            self.process_event_stream_item(EventStreamItem::ExecutionEvent {
                                seqno,
                                event: event.clone()
                            });
    ...
    fn process_event_stream_item(&mut self, item: EventStreamItem) {
        self.sessions.iter().for_each(|(_, session)| {
            session.subscriptions.iter().for_each(|(id, info)| {
                if matches!(info.kind, SubscriptionKind::MonadEventStream) {
                    let _ = session.conn_tx.send(WebSocketSessionCommand::PublishMessage {
                        messages: vec![EthSubscribeResult{
                            subscription: id.0,
                            result: SubscriptionResult::MonadEventStream(item.clone()),
                        }]
                    });
                }
            })
