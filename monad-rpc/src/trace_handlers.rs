...
mod tests {
    ...
    async fn test_build_call_tree() {
        ...
        assert_eq!(count_sub_call_frames(&*result.borrow()), 4);

        let flatten = flatten_call_tree_with_paths(result.borrow().clone());
        flatten
            .into_iter()
            .enumerate()
            .for_each(|(idx, (path, frame))| match idx {
                0 => {
                    assert!(path.is_empty());
                    assert_eq!(count_sub_call_frames(&frame), 4);
                }
                1 => {
                    assert_eq!(path, vec![0]);
                    assert_eq!(count_sub_call_frames(&frame), 3);
                }
                2 => {
                    assert_eq!(path, vec![0, 0]);
                    assert_eq!(count_sub_call_frames(&frame), 2);
                }
                3 => {
                    assert_eq!(path, vec![0, 0, 0]);
                    assert_eq!(count_sub_call_frames(&frame), 0);
                }
                4 => {
