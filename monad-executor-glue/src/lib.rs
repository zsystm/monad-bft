...
#[derive(Clone, PartialEq, Eq)]
pub enum MempoolEvent<ST, SCT, EPT: ExecutionProtocol>
where
    ST: CertificateSignatureRecoverable,
    SCT: SignatureCollection<NodeIdPubKey = CertificateSignaturePubKey<ST>>,
{
...
impl<ST, SCT, EPT: ExecutionProtocol> Encodable for MempoolEvent<ST, SCT, EPT>
where
    ST: CertificateSignatureRecoverable,
    SCT: SignatureCollection<NodeIdPubKey = CertificateSignaturePubKey<ST>>,
{
...
impl<ST, SCT, EPT: ExecutionProtocol> Decodable for MempoolEvent<ST, SCT, EPT>
where
    ST: CertificateSignatureRecoverable,
    SCT: SignatureCollection<NodeIdPubKey = CertificateSignaturePubKey<ST>>,
{
...
impl<ST, SCT, EPT: ExecutionProtocol> Debug for MempoolEvent<ST, SCT, EPT>
where
    ST: CertificateSignatureRecoverable,
    SCT: SignatureCollection<NodeIdPubKey = CertificateSignaturePubKey<ST>>,
{
...
impl<SCT: SignatureCollection> Encodable for KnownPeer<SCT> {
...
impl<SCT: SignatureCollection> Decodable for KnownPeer<SCT> {
    fn decode(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
        let mut payload = alloy_rlp::Header::decode_bytes(buf, true)?;

        let node_id = NodeId::<SCT::NodeIdPubKey>::decode(&mut payload)?;
...
where
    SCT: SignatureCollection,
{
    pub known_peers: Vec<KnownPeer<SCT>>,
}
    ...
    EPT: ExecutionProtocol,
{
    fn encode(&self, out: &mut dyn BufMut) {
        match self {
            ...
            Self::ExecutionResultEvent(event) => {
    ...
    EPT: ExecutionProtocol,
{
    fn decode(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
        ...
        match u8::decode(&mut payload)? {
            ...
            5 => Ok(Self::ExecutionResultEvent(ExecutionResult::<EPT>::decode(
                &mut payload,
            )?)),
            6 => Ok(Self::ControlPanelEvent(ControlPanelEvent::<SCT>::decode(
                &mut payload,
            )?)),
            7 => Ok(Self::TimestampUpdateEvent(u128::decode(&mut payload)?)),
            8 => Ok(Self::StateSyncEvent(
                StateSyncEvent::<ST, SCT, EPT>::decode(&mut payload)?,
            )),
            9 => Ok(Self::ConfigEvent(ConfigEvent::<SCT>::decode(&mut payload)?)),
            _ => Err(alloy_rlp::Error::Custom(
