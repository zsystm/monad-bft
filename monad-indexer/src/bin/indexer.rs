...
use monad_block_persist::{
    is_valid_bft_block_header_path, is_valid_bft_payload_path, FileBlockPersist,
};
...
fn new_block_payloads(bft_block_payload_path: &Path) -> impl Stream<Item = OsString> {
    let inotify = Inotify::init().expect("error initializing inotify");
    inotify
        .watches()
