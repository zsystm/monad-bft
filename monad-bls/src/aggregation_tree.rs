...
use alloy_rlp::{encode_list, BytesMut, Decodable, Encodable, Header, RlpDecodable, RlpEncodable};
use bitvec::prelude::*;
...
impl<PT: PubKey> SignatureCollection for BlsSignatureCollection<PT> {
    ...
    fn get_participants(
        ...
        assert_eq!(self.signers.0.len(), validator_mapping.map.len());

        let mut signers = HashSet::new();
        for (bit, (node_id, _)) in self.signers.0.iter().zip(validator_mapping.map.iter()) {
            if *bit {
