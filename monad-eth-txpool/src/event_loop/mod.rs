...
// TODO(andr-dev): Add comment here
...
where
    ...
    SBT: StateBackend,
{
    state: SharedEthTxPoolEventLoopState<SCT, SBT>,
    ...
    SBT: StateBackend + 'static,
{
    pub fn start(
        ...
        tx_expiry: Duration,
    ) -> EthTxPoolEventLoopClient<SCT, SBT> {
        let (events_tx, events_rx) = rtrb::RingBuffer::new(EVENTS_CAPACITY);

        let state = EthTxPoolEventLoopState::new_shared(block_policy, tx_expiry, events_rx);
