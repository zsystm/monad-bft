...
use monad_consensus_types::{
    block::{ExecutionProtocol, ExecutionResult, FinalizedHeader, ProposedExecutionResult},
    payload::{EthExecutionProtocol, EthHeader},
                        ...
                        StateRootRequest::Proposed {
                            ...
                            .map(|header| {
                                ExecutionResult::Proposed(ProposedExecutionResult {
                                    ...
                                    seq_num: *seq_num,
                                    round: *round,
                                    result: EthHeader(header),
                                })
                            }),
                        StateRootRequest::Finalized { seq_num } => handle
                            .get_finalized_eth_header(seq_num)
                            .map(|header| ExecutionResult::Finalized(*seq_num, EthHeader(header))),
                    };
