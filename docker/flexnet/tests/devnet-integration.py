    ...
    with net_config.start_topology(gen_config=False) as net:
        ...
        for i in range(TRANSACTION_COUNT):
            ...
            client.create_and_wait_for_transaction(
                Transaction(
                    sender_account, receiver_account, TRANSACTION_AMOUNT, GAS_AMOUNT
                )
            )

            print(f"sender balance = {client.get_account_balance(sender_account)}")
            print(
                f"expected sender balance = {INITIAL_ACCOUNT_BAL - (GAS_AMOUNT * MAX_FEE_PER_GAS + TRANSACTION_AMOUNT) * (i + 1)}"
            )
            print(f"receiver balance = {client.get_account_balance(receiver_account)}")
            client.expect_account_balance(
                sender_account,
                INITIAL_ACCOUNT_BAL
                - (GAS_AMOUNT * MAX_FEE_PER_GAS + TRANSACTION_AMOUNT) * (i + 1),
            )
