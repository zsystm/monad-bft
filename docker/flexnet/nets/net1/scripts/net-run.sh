...
# ethtx.sh
docker run --rm -v $flexnet_root:/monad monad-python-dev:latest python3 /monad/common/ethtx-gen.py $topology_json_path --tps 10 --num-tx 10000
...
# verify ledger
docker run --rm -v ./$flexnet_root:/monad monad-python bash -c "cd $vol_path_in_flexnet && python3 /monad/common/verify-ledger.py -c 4 -l ledger -n 10"
