...
if __name__ == "__main__":
    '''
    Non sequential nonce test:
    Send 10 transactions with valid nonces to a single node and expect them to run to
    completion.
    Send random invalid transactions after that to each node and expect none of them
    ...
    for nonce in range(NUM_TXNS):
        ...
        print(
            f"sent transaction to {rpc_addrs[0]}: from: {sender_account.address}, to: {receiver_account.address}, tx_hash: {tx_hash.hex()}, nonce: {nonce}")
        expected_tx_hashes.append(tx_hash)
    print("Finish sending valid transactions")

    # send invalid transactions with non sequential nonce to each node
    print("Start sending invalid transactions with nonces 20 to 40")
    for nonce in range(20, 40):
        # pick different rpc to service the transaction
        node_i = nonce % len(rpc_addrs)
        rpc_addr = rpc_addrs[node_i]
        w3_sender = w3_clients[node_i]

        receiver_account = Account.create()

        transfer_tx = make_transfer_tx(
            sender_address, receiver_account.address, nonce)
        signed_transfer = sender_account.sign_transaction(transfer_tx)
        tx_hash = w3_sender.eth.send_raw_transaction(
            signed_transfer.rawTransaction
        )
        print(
            f"sent transaction to {rpc_addr}: from: {sender_account.address}, to: {receiver_account.address}, tx_hash: {tx_hash.hex()}, nonce: {nonce}")
    print("Finish sending invalid transactions")
