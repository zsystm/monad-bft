...
RUN --mount=type=cache,target=${CARGO_ROOT}/registry    \
    ...
    CC=gcc-13 CXX=g++-13 cargo build --release --bin monad-node --features full-node --bin monad-keystore --bin debug-node --example wal2json --example wal-tool --example forkpoint-reset && \
    ...
    mv target/release/examples/forkpoint-reset forkpoint-reset && \
...
COPY --from=builder /usr/src/monad-bft/forkpoint-reset /usr/local/bin/forkpoint-reset
